...
You are responsible for implementing the chatkit.store.
Store class using the data store of your choice.
...
The recommended approach for relational databases is to serialize models into JSON-typed columns instead of separating model fields across multiple columns.
"""

from typing import Any, Generic, Literal, Dict, List
...
class MyDataStore(Store[dict[str, Any]]):
    ...
    async def load_threads(
        ...
        threads = sorted(
            (state.thread for state in self._threads.values()),
